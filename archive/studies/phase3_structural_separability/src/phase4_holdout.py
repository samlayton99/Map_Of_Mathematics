#!/usr/bin/env python3
"""Phase 4 certification: pre-registered holdout evaluation of the exact
...
def main():
    ...
    def variant_cands(vname, r):
        ...
        lb = {c for c in loadbearing(r) if pr[c]}
        if vname == "V5":
            return lb, {c for c in lb if indeg_v[c] <= 1}
        if vname == "V5z":
            return expand_fix(lb, r), None
        raise ValueError(vname)

    variants = ("V4hist", "V4b", "V5", "V5z")
    ...
    with open(os.path.join(DATA, "phase4_holdout_results.json"), "w") as f:
