...
def main():
    ...
    recalls, losses = [], {"not-in-term": 0, "background-slot": 0,
                           "claims-filter:not-Prop(def/construction)": 0,
    ...
    for r in holdout:
        ...
        for c in gt - moves:
            if c not in vset:
                ...
                why = "claims-filter:not-Prop(def/construction)"
            else:
    ...
    with open(os.path.join(DATA, "phase4_provenance_recall_unfiltered.json"), "w") as f:
