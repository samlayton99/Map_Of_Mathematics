#!/usr/bin/env python3
"""Phase 4 certification, round 2 (seed 20260821; rounds 1's sample is now
development data too — its diagnosis motivated the corrected claims filter).

Pre-registered before this run:
  V4hist   replication anchor (unchanged)
  V5p      exact claims filter: pr AND kind not in {constructor, recursor}
           (intro/elim rules are the logical form of goals, not argument
           steps; constructor/recursor are KERNEL kinds, not names);
           single-use kept as attribute, no inlining; rank (new, depth)
  V5pz     V5p with the judge's zoom display semantics: when rank-1 is
           single-use, the opened view is judged instead (recursive, cycle-
           safe, <=8 openings)
...
def main():
    ...
    for vname in variants:
        ...
        for r in roots:
            ...
            cs = cand[vname][r]
            nf = newflag[r]
            ranked = sorted(cs, key=lambda c: (not nf.get(c, True), -int(depth[c])))
            if vname == "V5pz":
                claim = lambda c: pr[c] and kinds[c] not in ("constructor", "recursor")
                opened = set()
                for _ in range(8):
                    ...
                    ranked = sorted((set(ranked) - {top}) | inner,
    ...
    with open(os.path.join(DATA, "phase4_holdout2_results.json"), "w") as f:
