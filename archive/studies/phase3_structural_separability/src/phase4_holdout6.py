#!/usr/bin/env python3
"""Phase 4 certification, round 6 (seed 20260828; seeds 20260819-27 all
excluded as development data, including the descriptive accounting samples).

Registered before running:
  V5v   round-5 certified system (anchor, 7th disjoint sample)
  V6    V5v + forwarder attribution, gated on the machine-generated flag:
        a candidate with gen=true (no source declaration range - recorded
        environment fact) whose own substantive move set is exactly ONE
        non-bookkeeping claim P is displayed and scored as P ("P, rewritten
        form"); recursive <=3 with cycle guard. Kernel-honest: a derived
        form must prove itself FROM its original, so the original is in its
        proof term; human-named corollaries are never dissolved (gen=false).
Bar declared in advance: V6 >= 0.93 top-1 proxy on its own denominator.
"""
...
def main():
    ...
    both = set(live_top1["V5v"]) & set(live_top1["V6"])
    out["matched_denominator"] = {
        "n_both_live": len(both),
        "V5v_top1_matched": round(float(np.mean([live_top1["V5v"][r] for r in both])), 4),
    ...
    with open(os.path.join(DATA, "phase4_holdout6_results.json"), "w") as f:
