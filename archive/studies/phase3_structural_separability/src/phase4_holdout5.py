...
def main():
    ...
    both = set(live_top1["V4hist"]) & set(live_top1["V5v"])
    out["matched_denominator"] = {
        "n_both_live": len(both),
        "V4hist_top1": round(float(np.mean([live_top1["V4hist"][r] for r in both])), 4),
        "V5v_top1": round(float(np.mean([live_top1["V5v"][r] for r in both])), 4)}
    print("matched_denominator =", json.dumps(out["matched_denominator"]))
    with open(os.path.join(DATA, "phase4_holdout5_results.json"), "w") as f:
