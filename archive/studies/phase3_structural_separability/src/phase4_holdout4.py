#!/usr/bin/env python3
"""Phase 4 certification, round 4 (seed 20260824; seeds 20260819/20/21/22
are development data, all excluded).

Round-3 falsified the rfl/propext hypothesis (demotion moved +0.09 points).
Mechanism identified instead: top-only zoom opens rank-1 containers only;
the historical inlining opened EVERY single-use candidate (breadth), which
is also the judge's display model (single-use = auto-open hint wherever it
sits). Registered here:
  V4hist   replication anchor (5th disjoint sample)
  V5pzb    round-3 formulation (carried, for continuity)
  V5pzB    breadth zoom: every single-use candidate is expanded to fixpoint
           (cycle-safe); containers keep their labels in the membership set
           but ranking judges the opened forest. Bar: >= 0.88.
...
def main():
    ...
    def variant_cands(vname, r):
        ...
        if vname == "V5pzB":
            out, frontier, seen = set(lb), {c for c in lb if indeg_v[c] <= 1}, set()
            for _ in range(64):
                frontier -= seen
                if not frontier:
                    break
                ...
                new = set()
                for c in frontier:
                    out.discard(c)   # container label kept in membership, not ranking
                    for d in loadbearing(c):
                        if d != r and claim(d):
                            new.add(d)
                out |= new
    ...
    for vname in variants:
        ...
        for r in roots:
            ...
            if vname == "V5pzb":
                ...
                key = lambda c: (not nf.get(c, True), -int(depth[c]))
    ...
    with open(os.path.join(DATA, "phase4_holdout4_results.json"), "w") as f:
