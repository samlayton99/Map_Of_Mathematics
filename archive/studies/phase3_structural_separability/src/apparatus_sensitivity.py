...
DEV_SEED = 20260819
HOLDOUT_SEED = 20260819  # DEV: sensitivity analysis, not certification
NSAMP = 2400
BATCH = 600
LOAD_ROLES = (0, 1, 2, 7)
LAMBDA = 20
...
def main():
    ...
    for i in range(n):
        ...
        for k in ks:
            nproof[k] += 1
    def make_apparatus(lam, floor):
        return (is_concept & ~bare_prop & (univ < 0.02) & (nproof > floor)
                & (nproof > lam * (inherited + 1)))
    apparatus = make_apparatus(LAMBDA, NPROOF_FLOOR)
    ...
    SWEEP = [(lam, fl) for lam in (10, 15, 20, 30, 50) for fl in (50, 200, 500)]
    print("\nSENSITIVITY SWEEP (dev seed 20260819; descriptive, not certification)")
    print(f"{'lambda':>7} {'floor':>6} {'|app|':>6} {'top1_ext':>9} {'tactic':>7} {'verdicts':>9}")
    sweep_out = []
    for lam, fl in SWEEP:
        apparatus = make_apparatus(lam, fl)
        taint_cache.clear()
        fcr2, cause2, nverd = [], Counter(), 0
        for r in roots:
            if len(set(deps_v[r]) - {r}) < 3:
                continue
            kind, ranked, cats, _ = score("V8", r)
            if kind or not ranked:
                nverd += 1 if kind else 0
        ...
        t1 = float((lv == 1).mean())
        row = {"lambda": lam, "floor": fl, "n_apparatus": int(apparatus.sum()),
               "top1_extended": round(t1, 4), "tactic_blames": cause2.get("tactic", 0),
               "verdicts": nverd}
        sweep_out.append(row)
        print(f"{lam:>7} {fl:>6} {int(apparatus.sum()):>6} {t1:>9.4f} "
              f"{cause2.get('tactic', 0):>7} {nverd:>9}", flush=True)
    json.dump(sweep_out, open(os.path.join(DATA, "apparatus_sensitivity.json"), "w"), indent=1)
    apparatus = make_apparatus(LAMBDA, NPROOF_FLOOR); taint_cache.clear()
