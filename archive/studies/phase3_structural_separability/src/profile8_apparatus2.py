#!/usr/bin/env python3
"""DEVELOPMENT profiling #8: apparatus admits predicate-encoded vocabulary.

V8a  apparatus = DATA-sorted, non-universal, used>>stated  (profile7 winner)
V8c  apparatus = NOT-A-BARE-PROPOSITION, non-universal, used>>stated
     Rationale: some procedures encode goals in PREDICATES (NormNum.IsNat
     "a is the numeral n", Nat.ToInt.*), not data. Bare propositions
     (True/False) are excluded because they are already bookkeeping -- and
     they are the only Prop-sorted concepts with the used>>stated profile
     that are NOT somebody's apparatus.
No capsule rule in either (profile7: it hides real moves).

Original profile7 header follows.
...
def main():
    ...
    heavy = (is_concept & (univ < 0.02) & (nproof > NPROOF_FLOOR)
             & (nproof > LAMBDA * (inherited + 1)))
    apparatus_A = heavy & ~ps                  # data-sorted only
    apparatus_C = heavy & ~bare_prop           # anything but bare propositions
    print(f"apparatus A (data): {int(apparatus_A.sum())}  "
          f"C (non-bare-prop): {int(apparatus_C.sum())}", flush=True)
    extra = np.where(apparatus_C & ~apparatus_A)[0]
    print(f"admitted by C only ({len(extra)}):", flush=True)
    for i in extra[np.argsort(-nproof[extra])][:40]:
        print(f"   {names[i]:<70} stated={int(inherited[i]):>6} used={int(nproof[i]):>8}", flush=True)
    apparatus = apparatus_A

    taint_caches = {"A": {}, "C": {}}
    def tainted_v(which, c):
        cache = taint_caches[which]
        r = cache.get(c)
        if r is None:
            app = apparatus_A if which == "A" else apparatus_C
            r = any(app[k] for k in nonuniv_ingredients(c))
            cache[c] = r
        return r
    def tainted(c):
        return tainted_v("A", c)
    ...
    def score(vname, r):
        ...
        if vname == "V6":
            ...
            which = "A" if vname == "V8a" else "C"
            cs = set(cand8[r])
            dem = lambda c: bookkeeping(c) or (
                tainted_v(which, c) and not (nonuniv_ingredients(c) & rs))
        nf = newflag[r]
    ...
    with open(os.path.join(DATA, "profile8_apparatus2.json"), "w") as fh:
