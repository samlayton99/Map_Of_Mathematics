#!/usr/bin/env python3
"""Phase 4 certification, round 9 (seed 20260831; seeds 20260819-30 all
...
REGISTERED BEFORE RUNNING.
  ...
  V8  V6 + the apparatus measure:
      ...
      bare      a Prop-sorted concept of ARITY 0 (`True`, `False`) -- it IS
                a proposition, as opposed to a predicate ABOUT something
                (`Function.Injective`, `Even`), which takes arguments.
                Round 8 used "mentions no constants" as the proxy for arity
                0 and was FALSIFIED by it: a predicate over abstract types
                mentions no constants either, so `Function.Injective` was
                misfiled as bare and real moves ("an equivalence is
                injective") were demoted to bookkeeping. Arity is now read
                from the kernel telescope (dump v7, field `ar`).
...
Round 8 (seed 20260830, burned) ran this spec with the defective bare test:
bars 1 and 2 PASSED (ext 0.9221 -> 0.9351; tactic blames 19 -> 15) but bar 3
FAILED with 31 content demotions, 27 of them lists collapsing to a
definitional verdict through the misfiled predicates. Same bars here.
...
DEV_SEED = 20260819
HOLDOUT_SEED = 20260819  # DEV: ablation study, not certification
NSAMP = 2400
BATCH = 600
LOAD_ROLES = (0, 1, 2, 7)
LAMBDA = 20
...
def load():
    idx, names = {}, []
    deps_v, deps_t, kinds, classes, vo, pr, gen, ps, ar = [], [], [], [], [], [], [], [], []
    def nid(nm):
        i = idx.get(nm)
        if i is None:
    ...
    return idx, names, deps_v, deps_t, kinds, classes, vo, pr, gen, ps, ar


def main():
    idx, names, deps_v, deps_t, kinds, classes, vo, pr, gen, ps, ar = load()
    n = len(names)
    ...
    print(f"bare propositions: {int(bare_prop.sum())} -> "
          f"{[names[i] for i in np.where(bare_prop)[0][:10]]}", flush=True)
    ...
    for sd in (20260828, 20260829, 20260830):
        p = np.array([i for i in pool if i not in dev])
        dev |= set(np.random.default_rng(sd).choice(p, size=NSAMP, replace=False).tolist())
    roots = np.random.default_rng(20260819).choice(pool, size=NSAMP, replace=False).tolist()
    print(f"holdout sample: {len(roots)} (disjoint: {len(set(roots) & dev) == 0})", flush=True)

    allrefs = {}
    for r in roots:
        allrefs[r] = {d for d in deps_v[r] if d != r}
    cand6, cand8, need = {}, {}, {}
    for r in roots:
    ...
    # ---------------- ABLATION STUDY (dev seed; descriptive) ----------------
    def run(cfg):
        """cfg keys: position, claims, logic, apparatus, attribution, zoom,
        stmtworld, depthkey  (all bool)"""
        fcr2, cause2, nverd = [], Counter(), 0
        for r in roots:
            if len(set(deps_v[r]) - {r}) < 3:
                continue
            rn = names[r]
            rs = stmt_concepts(r)
            base = loadbearing(r) if cfg["position"] else allrefs[r]
            cs = {c for c in base if claimf(c)} if cfg["claims"] else set(base)
            if cfg["attribution"]:
                cs = {attr8(c) for c in cs}
            nf = newflag[r]
            def dem(c):
                d = False
                if cfg["logic"]:
                    d = d or bookkeeping8(c)
                if cfg["apparatus"]:
                    d = d or (tainted(c) and not (nonuniv_ingredients(c) & rs))
                return d
            def key(c):
                k = []
                if cfg["logic"] or cfg["apparatus"]:
                    k.append(dem(c))
                if cfg["stmtworld"]:
                    k.append(not nf.get(c, True))
                if cfg["depthkey"]:
                    k.append(-int(depth[c]))
                return tuple(k)
            if cs and (cfg["logic"] or cfg["apparatus"]) and all(dem(c) for c in cs):
                nverd += 1; fcr2.append(0); continue
            ranked = sorted(cs, key=key)
            if cfg["zoom"]:
                opened = set()
                for _ in range(8):
                    ...
                    ranked = sorted((set(ranked) - {top}) | inner, key=key)
            if not ranked:
                fcr2.append(0); continue
            c2 = {c: category2(c, rn) for c in ranked}
            fcr2.append(next((k + 1 for k, c in enumerate(ranked) if c2[c] == "content"), 99))
            if c2[ranked[0]] != "content":
                cause2[c2[ranked[0]]] += 1
        a2 = np.array(fcr2); lv = a2[a2 > 0]
        return {"top1": round(float((lv == 1).mean()), 4),
                "tactic": cause2.get("tactic", 0), "glue": cause2.get("glue", 0),
                "verdicts": nverd, "n_live": int((a2 > 0).sum())}

    FULL = dict(position=True, claims=True, logic=True, apparatus=True,
                attribution=True, zoom=True, stmtworld=True, depthkey=True)
    configs = [("V8 full", FULL)]
    for k, label in (("position", "-position filter"), ("claims", "-claims filter"),
                     ("logic", "-logic-only demotion"), ("apparatus", "-apparatus demotion"),
                     ("attribution", "-attribution"), ("zoom", "-zoom"),
                     ("stmtworld", "-statement-world key"), ("depthkey", "-depth key")):
        c = dict(FULL); c[k] = False
        configs.append((label, c))
    configs.append(("depth only (no filters, no tiers)",
                    dict(position=False, claims=False, logic=False, apparatus=False,
                         attribution=False, zoom=False, stmtworld=False, depthkey=True)))
    configs.append(("claims+depth only",
                    dict(position=False, claims=True, logic=False, apparatus=False,
                         attribution=False, zoom=False, stmtworld=False, depthkey=True)))
    configs.append(("position+claims+depth",
                    dict(position=True, claims=True, logic=False, apparatus=False,
                         attribution=False, zoom=False, stmtworld=False, depthkey=True)))
    configs.append(("position+claims+stmtworld+depth (V4-like)",
                    dict(position=True, claims=True, logic=False, apparatus=False,
                         attribution=False, zoom=False, stmtworld=True, depthkey=True)))
    configs.append(("position+claims+logic+stmtworld+depth (V5v-like)",
                    dict(position=True, claims=True, logic=True, apparatus=False,
                         attribution=False, zoom=False, stmtworld=True, depthkey=True)))
    print("\nABLATION (dev seed 20260819, extended grader; descriptive)")
    print(f"{'config':<46} {'top1':>7} {'tactic':>7} {'glue':>6} {'verdicts':>9} {'n_live':>7}")
    res = []
    for label, cfg in configs:
        rr = run(cfg); rr["config"] = label; res.append(rr)
        print(f"{label:<46} {rr['top1']:>7.4f} {rr['tactic']:>7} {rr['glue']:>6} "
              f"{rr['verdicts']:>9} {rr['n_live']:>7}", flush=True)
    json.dump(res, open(os.path.join(DATA, "v8_ablation.json"), "w"), indent=1)
    ...
    with open(os.path.join(DATA, "phase4_holdout9_results.json"), "w") as fh:
