...
DUMP4 = os.path.join(SCRATCH2, "mathlib_deps7.jsonl")
...
def main():
    ...
    recalls, losses = [], {"not-in-term": 0, "background-slot": 0,
                           "claims-filter:statement-vocabulary": 0,
                           "claims-filter:unfolding-move(NOT expressible)": 0,
                           "claims-filter:ctor-or-recursor": 0}
    loss_examples = []
    evaluated = 0
    ...
    for r in holdout:
        ...
        for cn in refs:
            ...
            if c is None or c == r:
                continue
            gt.add(c)                      # NO filter applied to the answer key
        if len(gt) < 1:
            continue
        evaluated += 1
        ...
        for c in gt - moves:
            if c not in vset:
                ...
                why = ("claims-filter:statement-vocabulary"
                       if c in set(deps_t[r])
                       else "claims-filter:unfolding-move(NOT expressible)")
            else:
                why = "claims-filter:ctor-or-recursor"
            losses[why] += 1
            if len(loss_examples) < 60:
                loss_examples.append({"decl": nm, "lost": names[c],
                                      "why": why, "kind": kinds[c]})
    out = {
        ...
        "note": "UNFILTERED ground truth = every elaborator-resolved source "
                "identifier, no claims filter applied to the answer key; this is the honest recall of the SHIPPED pipeline including definition 2"}
    with open(os.path.join(DATA, "phase4_recall_loss_split.json"), "w") as f:
